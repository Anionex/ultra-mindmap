DIRECT_PROMPT = """\
请基于以下文档内容，生成一张层级清晰、主题归纳明确、适合快速把握全文结构的思维导图。你的目标是把文章压缩成一张"知识地图"，让读者能够迅速理解这篇文章的核心主题、主要模块、关键内容，以及各部分之间的层级关系。
...
需要时继续向下拆分更细的层级，整体层级最多{max_depth}层（使用 # 到 {max_depth_marks} 表示）。请根据内容复杂度自然决定展开深度，使层级既完整又紧凑。使用中文语言。
...
节点标题必须尽量保持为短语、术语或单个概念，不要写成“标签 + 解释”的形式。除非原文术语本身如此，否则不要在节点标题中使用冒号（: 或 ：）、破折号（-、--、——）或类似分隔符来追加说明，不要把完整判断句、解释句、定义句、因果句写进节点标题。
